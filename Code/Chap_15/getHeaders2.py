...
#    Note: Display just the H3 JFrame HTML headers in a scrollable list
#   Usage: set jsoup=-wsadmin_classpath C:\Programs\jsoup\jsoup-1.8.1.jar
#          wsadmin %jsoup% -f getHeaders2.py
#            or
#          set JYTHONPATH=C:\Programs\jsoup\jsoup-1.8.1.jar
#          jython getHeaders2.py
# History:
...
class FormattingVisitor( NodeVisitor ) :
    ...
    def head( self, node, depth ) :
        ...
        if name == 'h3' :
            if node.hasText() :
                self.append( '%s: %s' % ( name, node.text() ) )
...
#-------------------------------------------------------------------------------
class getHeaders2( java.lang.Runnable ) :
...
if __name__ in [ '__main__', 'main' ] :
    EventQueue.invokeLater( getHeaders2() )
