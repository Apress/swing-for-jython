...
#    Note: This iteration adds a PropertyChangeListener event handler method
#          (i.e., propChange) to monitor the divider location / size.
#   Usage: wsadmin -wsadmin_classpath jsoup-1.8.1.jar -f javadocInfo_04.py
#            or
#     ...  JYTHONPATH must include path to jsoup*.jar
#          e.g., set JYTHONPATH=C:\Programs\jsoup\jsoup-1.8.1.jar;
#          jython javadocInfo_04.py
# History:
...
try :
    ...
    if 'AdminConfig' in dir() :
        print 'e.g.: wsadmin -wsadmin_classpath jsoup-1.8.1.jar -f javadocInfo_04.py'
    else :
        ...
        print 'jython javadocInfo_04.py'
    sys.exit()
...
#-------------------------------------------------------------------------------
class javadocInfo_04( java.lang.Runnable ) :
    ...
    #---------------------------------------------------------------------------
    # Name: propChange()
    # Role: PropertyChangeListener event handler for JSplitPane
    #---------------------------------------------------------------------------
    def propChange( self, pce ) :
        src  = pce.getSource()         # Should be our JSplitPane
        prop = pce.getPropertyName()
        if prop == JSplitPane.LAST_DIVIDER_LOCATION_PROPERTY :
            curr = src.getDividerLocation()
            last = pce.getNewValue()
            prev = pce.getOldValue()
...
if __name__ in [ '__main__', 'main' ] :
    EventQueue.invokeLater( javadocInfo_04() )
    if 'AdminConfig' in dir() :
