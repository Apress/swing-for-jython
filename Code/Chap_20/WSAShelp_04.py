...
#          method details.  The classes and methods to highlight text were
#          added but we don't yet have a way to invoke them.  However, we now
#          have an event handler that is called when one of the tabbed panes is
#          selected.
#    Note: This requires a WebSphere Application Server environment.
#   Usage: wsadmin -f WSAShelp_04.py
# History:
...
#-------------------------------------------------------------------------------
class WSAShelp_04( java.lang.Runnable ) :
    ...
    def run( self ) :
        ...
        tabs = JTabbedPane( stateChanged = self.tabPicked )
    ...
    def tabPicked( self, event ) :
        ...
        print pane.getTitleAt( pane.getSelectedIndex() )
...
if __name__ in [ '__main__', 'main' ] :
    if 'AdminConfig' in dir() :
        EventQueue.invokeLater( WSAShelp_04() )
        raw_input( '\nPress <Enter> to terminate the application:\n' )
    else :
        print '\nError: This script requires a WebSphere environment.'
        print 'Usage: wsadmin -f WSAShelp_04.py'
else :
