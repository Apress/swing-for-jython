...
#          This iteration adds code to show the AdminTask command groups using
#          a tree (JTree) structure.
#    Note: This requires a WebSphere Application Server environment.
#   Usage: wsadmin -f WSAShelp_11.py
# History:
...
Purpose: A wsadmin Jython script demonstrating some techniques for graphically
 ...
 Author: Robert A. (Bob) Gibson <bgibson@us.ibm.com>
  ...
  Usage: wsadmin -f WSAShelp_11.py
Example: ./wsadmin.sh -f WSAShelp_11.py\n
Version: 0.10
Updated: 01 Jan 2013
'''
...
#-------------------------------------------------------------------------------
class WSAShelp_11( java.lang.Runnable ) :
...
#               print message
    ...
    def pickATgroup( self, event ) :
        ...
        if node :
            ...
            helpText = AdminTask.help( choice ).expandtabs()
            dashes   = '#' + ( '-' * 60 ) + '\n'
            header   = '# print AdminTask.help( "%s" )\n' % choice
            message  = dashes + header + dashes + '\n' + helpText
        else :
...
if __name__ in [ '__main__', 'main' ] :
    if 'AdminConfig' in dir() :
        EventQueue.invokeLater( WSAShelp_11() )
        raw_input( '\nPress <Enter> to terminate the application:\n' )
    else :
        print '\nError: This script requires a WebSphere environment.'
        print 'Usage: wsadmin -f WSAShelp_11.py'
else :
