...
#          This iteration adds code to show the AdminTask command list.
#    Note: This requires a WebSphere Application Server environment.
#   Usage: wsadmin -f WSAShelp_10.py
# History:
...
Purpose: A wsadmin Jython script demonstrating some techniques for graphically
 ...
 Author: Robert A. (Bob) Gibson <bgibson@us.ibm.com>
  ...
  Usage: wsadmin -f WSAShelp_10.py
Example: ./wsadmin.sh -f WSAShelp_10.py\n
Version: 0.10
Updated: 01 Jan 2013
'''
...
AT_USAGE = '''
- The input field can be used to filter the commands to be listed.\n
- Selecting a command will have it's help text displayed in this pane.
'''
...
class ATgroupsTask( SwingWorker ) :
    ...
    # Role: Constructor
    #---------------------------------------------------------------------------
    def __init__( self, jlist, menuItem ) :
        SwingWorker.__init__( self )
        self.groups = jlist
        self.menuItem = menuItem
    ...
    def doInBackground( self ) :
        try :
            data = AdminTask.help( '-commandGroups' ).splitlines()
            model = DefaultListModel()
            for line in data[ 1: ] :
                if line :
                    mo = re.match( '([a-zA-Z ]+) -', line )
                    if mo :
                        model.addElement( mo.group( 1 ) )
            self.groups.setModel( model )
        except :
...
#-------------------------------------------------------------------------------
class WSAShelp_10( java.lang.Runnable ) :
    ...
    def MenuBar( self ) :
        ...
        #-----------------------------------------------------------------------
        # list to hold list of AdminTask commands
        #-----------------------------------------------------------------------
        self.ATcommands = []
        #-----------------------------------------------------------------------
        ...
        groupMI = task.add(
            JMenuItem(
                '-commandGroups',
                enabled = 0,
                actionPerformed = self.TBD
            )
        )
        #-----------------------------------------------------------------------
        # JList to hold list of AdminTask commandGroups
        #-----------------------------------------------------------------------
        self.ATgroups = JList(
            selectionMode = ListSelectionModel.SINGLE_SELECTION
        )
        ...
        #-----------------------------------------------------------------------
        ATgroupsTask( self.ATgroups, groupMI ).execute()
    ...
    def pickATcmd( self, event ) :
        ...
        if not event.getValueIsAdjusting() :
            ...
            if index > -1 :
                ...
                message = AdminTask.help( choice ).expandtabs()
#               print message
                dashes  = '#' + ( '-' * 60 ) + '\n'
...
if __name__ in [ '__main__', 'main' ] :
    if 'AdminConfig' in dir() :
        EventQueue.invokeLater( WSAShelp_10() )
        raw_input( '\nPress <Enter> to terminate the application:\n' )
    else :
        print '\nError: This script requires a WebSphere environment.'
        print 'Usage: wsadmin -f WSAShelp_10.py'
else :
