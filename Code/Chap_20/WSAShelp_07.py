...
#          method details.  This iteration adds text highlighting for both the
#          top and bottom portions of the splitpane (including the table text).
#    Note: This requires a WebSphere Application Server environment.
#   Usage: wsadmin -f WSAShelp_07.py
# History:
...
class methRenderer( DefaultTableCellRenderer ) :
    ...
    # Name: __init__()
    # Role: Constructor
    # Note: Instantiate one JLabel() to be reused each time a cell is rendered
    #---------------------------------------------------------------------------
    def __init__( self ) :
    ...
    def getTableCellRendererComponent(
        ...
        if pWidth :                    # Has it been set?
            ...
            #-------------------------------------------------------------------
            hiHTML = '<font bgcolor=yellow>%s</font>'
            pWidth -= 3
...
class tableTask( SwingWorker ) :
    ...
    #---------------------------------------------------------------------------
    def __init__( self, helpText, splitPane, tPanes ) :
...
#-------------------------------------------------------------------------------
class WSAShelp_07( java.lang.Runnable ) :
    ...
    # Note: The width of 640 is used so that the AdminTask.help() won't wrap
    #---------------------------------------------------------------------------
    ...
    def run( self ) :
        ...
        for name, obj in objs :
            ...
            if name in [ 'wsadmin', 'AdminTask' ] :
                ...
                tableTask( meth, splitPane, self.tPanes[ name ] ).execute()
                tabs.addTab(
                    name,
                    splitPane
                )
...
if __name__ in [ '__main__', 'main' ] :
    if 'AdminConfig' in dir() :
        EventQueue.invokeLater( WSAShelp_07() )
        raw_input( '\nPress <Enter> to terminate the application:\n' )
    else :
        print '\nError: This script requires a WebSphere environment.'
        print 'Usage: wsadmin -f WSAShelp_07.py'
else :
