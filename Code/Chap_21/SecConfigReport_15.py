...
#   Usage: wsadmin -f SecConfigReport_15.py
# History:
#   date    who  ver   Comment
# --------  ---  ---   ----------
# 14/05/26  rag  0.15  Add - Character oriented progress indicator on startup
# 14/05/26  rag  0.14  Fix - Correct collapse section bug
# 14/05/26  rag  0.13  Add - Double click to expand / collapse a section
...
Purpose: A wsadmin Jython script demonstrating some techniques for graphically
 ...
 Author: Robert A. (Bob) Gibson <bgibson@us.ibm.com>
  ...
  Usage: wsadmin -f SecConfigReport_15.py
Example: ./wsadmin.sh -f SecConfigReport_15.py\n
Version: 0.15
Updated: 26 May 2014
'''
...
#-------------------------------------------------------------------------------
# Name: reportTask
# Role: Background processing to display an indeterminate progress bar while the
#       main thread is calling AdminTask.generateSecConfigReport()
# Note: Instances of SwingWorker are not reusuable, new ones must be created.
#-------------------------------------------------------------------------------
class reportTask( SwingWorker ) :

    #---------------------------------------------------------------------------
    # Name: doInBackground()
    # Role: Call the AdminConfig scripting object in the background
    # Note: The first two lines of the report are discarded
    #---------------------------------------------------------------------------
    def doInBackground( self ) :
        self.results = AdminTask.generateSecConfigReport().splitlines()[ 2: ]

    #---------------------------------------------------------------------------
    # Name: getResults()
    # Role: Returns the result of calling the AdminTask command
    #---------------------------------------------------------------------------
    def getResults( self ) :
        return self.results
...
class sectionFilter( RowFilter ) :
    ...
    def include( self, entry ) :
        ...
        section  = model.getSectionNumber( row )
        result   = model.isRowVisible( row ) or model.isSectionVisible( section )
...
class reportTableModel( DefaultTableModel ) :
    ...
    # Role: Return the number of sections present
    #---------------------------------------------------------------------------
    ...
    # Role: Return the section number of the specified row, or -1 if row doesn't
    #       exist
    #---------------------------------------------------------------------------
    def getSectionNumber( self, row ) :
        try :
            result = self.sectionNumber[ row ]
        except :
            result = -1
        return result
    ...
    # Role: Return boolean value indicating whether the specified section number
    #       is visible, or not.
    # Note: See sectionFilter class
    #---------------------------------------------------------------------------
    def isSectionVisible( self, sectionNum ) :
        try :
            result = self.sectionVisible[ sectionNum ]
        except :
            result = 0
        return result
    ...
    #---------------------------------------------------------------------------
    # Name: isRowVisible()
    # Role: Used by sectionFilter to determine if the specified row should be
    #       visible
    #---------------------------------------------------------------------------
    ...
    #---------------------------------------------------------------------------
    # Name: setSectionVisible()
    # Role: Indicate whether the specified row is visible, or not
    #---------------------------------------------------------------------------
    def setSectionVisible( self, sectionNum, trueFalse ) :
        self.sectionVisible[ sectionNum ] = trueFalse

    #---------------------------------------------------------------------------
    # Name: setRowVisible()
    # Role: Indicate whether the specified row is visible, or not
    #---------------------------------------------------------------------------
    def setRowVisible( self, row, trueFalse ) :
        ...
        if value :
            ...
            if findText :
                if result.find( findText ) > -1 :
                    result = result.replace( findText, '<font bgcolor="green" color="yellow">%s</font>' % findText )
...
#-------------------------------------------------------------------------------
# Name: SecConfigReport_15()
# Role: Used to demonstrate how to create, and display a JFrame instance
# Note: This class should be instantiated on the Swing Event Dispatch Thread
#-------------------------------------------------------------------------------
class SecConfigReport_15( java.lang.Runnable ) :
    ...
    #---------------------------------------------------------------------------
    # Name: clicker()
    # Role: Event handler for table Mouse Adapter
    # Note: Looking for double-click to toggle section visibility
    #---------------------------------------------------------------------------
    def clicker( self, event ) :
        if event.getClickCount() == 2 :
            table = event.getSource()
            model = table.getModel()
            view  = row = table.getSelectedRow()
            if view > -1 :
                row = table.convertRowIndexToModel( view )
                if model.getValueAt( row, 0 ).startswith( '_' ) :
                    sNum = model.getSectionNumber( row )
                    vis  = not model.isSectionVisible( sNum )
                    model.setSectionVisible( sNum, vis )
                    for row in range( row + 1, model.getRowCount() ) :
                        if model.getSectionNumber( row ) == sNum :
                            model.setRowVisible( row, vis )
                        else :
                            break
                else :
                    sNum = -1
    ...
    def collapse( self, event ) :
        table = self.table
        model = table.getModel()
        for s in range( model.getSectionCount() ) :
            model.setSectionVisible( s, 0 )
    ...
    def expand( self, event ) :
        ...
        model = table.getModel()
        for s in range( model.getSectionCount() ) :
            model.setSectionVisible( s, 1 )
        for row in range( model.getRowCount() ) :
            model.setRowVisible( row, 1 )
        table.getRowSorter().setRowFilter(
            sectionFilter()
        )
    ...
    def Find( self, event ) :
        table  = self.table
        cols   = table.getColumnModel().getColumnCount()
        model  = table.getModel()
        result = JOptionPane.showInputDialog(
            self.frame,                 # parentComponent
            'Text to be found:'         # message text
        )
        model.setFindText( result )
        for row in range( model.getRowCount() ) :
            visible = model.getValueAt(
                row,
                0
            ).startswith( '_' )
            if result and not visible :
                for col in range( cols ) :
                    ...
                    if val.find( result ) > -1 :
                        visible = 1
                        break
            model.setRowVisible( row, visible )
        table.getRowSorter().setRowFilter(
            sectionFilter()
        )
    ...
    def frameResized( self, ce ) :
        try :
            ...
            pWidth = int( width ) >> 2      # prefer 1/4 available width
            tcm    = table.getColumnModel() # Table Column Model
    ...
    def run( self ) :
        ...
        try :
            #-------------------------------------------------------------------
            # start the long running AdminTask call in the background, and
            # display a simple, character oriented, progress indication as
            # it executes.
            #-------------------------------------------------------------------
            task = reportTask()
            task.execute()
            ...
            while not task.isDone() :
                print '\b%s\b' % chars[ char ],
                sleep( 0.25 )
                char = ( char + 1 ) % len( chars )
            print '\b \b',
            info = task.getResults()
        except :
            print '\nError: %s\nvalue: %s' % sys.exc_info()[ :2 ]
            sys.exit()
...
if __name__ in [ '__main__', 'main' ] :
    EventQueue.invokeLater( SecConfigReport_15() )
