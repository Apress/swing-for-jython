...
#   Usage: wsadmin -f SecConfigReport_09.py
# History:
...
# Role: Define table data characteristics
#-------------------------------------------------------------------------------
class rowFinder( ChangeListener ) :

    #---------------------------------------------------------------------------
    # Name: __init__()
    # Role: Class constructor
    # Note: Saves a reference to the table
    #---------------------------------------------------------------------------
    ...
    #---------------------------------------------------------------------------
    # Name: stateChanged()
    # Role: ChangeListener change event handler
    #---------------------------------------------------------------------------
    def stateChanged( self, ce ) :
        vPort = ce.getSource()
        table = self.table
        rect  = vPort.getViewRect()
        first = table.rowAtPoint(
            Point( 0, rect.y )
        )
        last  = table.rowAtPoint(
            Point( 0, rect.y + rect.height - 1 )
        )
        print 'rows: %d..%d  isValid: %d' % (
            first,
            last,
            vPort.isValid()
        )
...
#-------------------------------------------------------------------------------
# Name: SecConfigReport_09()
# Role: Used to demonstrate how to create, and display a JFrame instance
# Note: This class should be instantiated on the Swing Event Dispatch Thread
#-------------------------------------------------------------------------------
class SecConfigReport_09( java.lang.Runnable ) :
    ...
    def run( self ) :
        ...
        scroller.getViewport().addChangeListener(
            rowFinder( table )
        )
        frame.add( scroller )
