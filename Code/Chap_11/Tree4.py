...
#    Role: Simple Jython Swing script that listens for tree node selection events
#    Note: A WebSphere Application Server environment is required.
#   Usage: wsadmin -f Tree4.py
# History:
...
#-------------------------------------------------------------------------------
class Tree4( java.lang.Runnable ) :
    ...
    def cellTree( self ) :
        ...
        return JTree( root, valueChanged = self.select )
    ...
    def select( self, event ) :
        ...
        if tree.getSelectionCount() :
            node = str( tree.getLastSelectedPathComponent() )
        else :
            node = '<none>'
        self.msg.setText( node )
...
if __name__ in [ '__main__', 'main' ] :
    if 'AdminConfig' in dir() :
        EventQueue.invokeLater( Tree4() )
        raw_input( '\nPress <Enter> to terminate the application:\n' )
    else :
        print '\nError: This script requires a WebSphere environment.'
        print 'Usage: wsadmin -f Tree4.py'
else :
